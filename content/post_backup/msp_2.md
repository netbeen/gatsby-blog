...
<ruby>模数转换器<rp></rp><rt>Analog-Digital Converter</rt></ruby>简称为 ADC，顾名思义，就是把模拟信号通过采样转换为数字信号。在单片机中，我们经常会用到一些传感器来实现人机交互。像在上一节中最基础的按钮就是一个人机交互的例子，表面上是人对按钮施加压力，内部却是按钮所在电路的通断，再深入一点就是按钮电位的高低。不过在按钮这个例子中，我们只有高电平和低电平，就像 0 和 1，是数字信号。但是其他的传感器，比如光敏电阻、拨码电位器这些，它们反映出来的数字并不是非 0 即 1，而是有一定范围的数据，这时候就需要 ADC 了。
